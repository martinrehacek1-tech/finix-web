...
const COLOR_START = new Color("#00e3c7");
const COLOR_MIDDLE = new Color("#009bd7");
const COLOR_END = new Color("#0751d9");

const HORIZONTAL_WEIGHT = 0.5;
const VERTICAL_WEIGHT = 0.5;
...
function applyFinixMaterial(root: Object3D) {
  ...
  root.traverse((child) => {
    ...
    for (let index = 0; index < position.count; index += 1) {
      ...
      const horizontal =
        size.x > 0
          ? (worldPosition.x - bounds.min.x) / size.x
          : 0.5;

      const vertical =
        size.y > 0
          ? (worldPosition.y - bounds.min.y) / size.y
          : 0.5;

      const gradientPosition = MathUtils.clamp(
        horizontal * HORIZONTAL_WEIGHT +
          (1 - vertical) * VERTICAL_WEIGHT,
        0,
        1,
      );
      ...
      if (gradientPosition < 0.46) {
        color
          .copy(COLOR_START)
          .lerp(
            COLOR_MIDDLE,
            gradientPosition / 0.46,
          );
      } else {
        color
          .copy(COLOR_MIDDLE)
    ...
    child.material = new MeshPhysicalMaterial({
      ...
      sheen: 0.06,
      sheenColor: new Color("#50eee5"),
      sheenRoughness: 0.5,
      emissive: new Color("#00131d"),
...
export default function FinixLogoModel({
  ...
  MathUtils.smootherstep(
    ...
    const interactionStrength = 1 - alignmentProgress;

    const idleYaw =
      Math.sin(elapsedRef.current * 0.34) * 0.12;

    const idlePitch =
      Math.sin(elapsedRef.current * 0.46) * 0.025;
